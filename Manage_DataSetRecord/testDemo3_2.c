  ...
  do {									\
    ...
    for(i = 0; i < schema->numAttr; i++)				\
      {									\
        Value *lVal, *rVal;                                             \
		char *lSer, *rSer; \
        AttrGetter(_lR, schema, i, &lVal);                                  \
        AttrGetter(_rR, schema, i, &rVal);                                  \
...
Schema *testSchema(void)
{
	...
	result = Schema_Creation(3, cpNames, cpDt, cpSizes, 1, cpKeys);
...
RC testTombstone(void)
{
	...
	TEST_CHECK(initRManager(NULL));
	TEST_CHECK(generate_table("test_table_r", schema));
...
Record * testRecord(Schema *schema, int a, char *b, int c)
{
	...
	TEST_CHECK(create_Record(&result, schema));

	MAKE_VALUE(value, DT_INT, a);
	TEST_CHECK(AttrSetter(result, schema, 0, value));
	freeVal(value);

	MAKE_STRING_VALUE(value, b);
	TEST_CHECK(AttrSetter(result, schema, 1, value));
	freeVal(value);

	MAKE_VALUE(value, DT_INT, c);
	TEST_CHECK(AttrSetter(result, schema, 2, value));
	freeVal(value);
